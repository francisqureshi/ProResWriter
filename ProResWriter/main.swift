...
Task {
    ...
    do {
        let gradedSegments = try await importProcess.importGradedSegments(from: segmentsDirectoryURL)
        print("✅ Successfully imported \(gradedSegments.count) graded segments")
        
        for segment in gradedSegments {
            print("📄 \(segment.fileName)")
            print("  📐 Resolution: \(Int(segment.resolution.width))x\(Int(segment.resolution.height))")
