                ...
                ScrollView(.horizontal, showsIndicators: true) {
                    ...
                    HStack(spacing: 0) {
                        ...
                        HStack(spacing: 0) {
                            ...
                            ResizeDivider { delta in
                                confidenceWidth = max(confidenceWidth + delta, minConfidenceWidth)
                            }
                        }
...
struct LinkingTableRowView: View {
    ...
    private var durationText: String {
        switch row.type {
        case .ocfParent:
            if let frames = row.ocfParent?.ocf.durationInFrames, let fps = row.ocfParent?.ocf.frameRate, fps > 0 {
                let seconds = Float(frames) / fps
                return String(format: "%.2fs", seconds)
            }
            return ""
        case .linkedSegment:
            if let frames = row.linkedSegment?.segment.durationInFrames, let fps = row.linkedSegment?.segment.frameRate, fps > 0 {
                let seconds = Float(frames) / fps
                return String(format: "%.2fs", seconds)
            }
            return ""
